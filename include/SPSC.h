...
public:
    ...
    explicit SPSC(size_type capacity = 1024, Alloc const& alloc = Alloc{})
        : _alloc{alloc},
            _capacity(capacity),
...
bool SPSC<T, Alloc>::push(T&& value) {
    if (_capacity == 0) {
        return false;
    }
    ...
    allocator_traits::construct(_alloc, _ring + idx, std::move(value));
    _push_cursor.store(push_cursor + 1, std::memory_order_release);
    return true;
}

template<typename T, typename Alloc>
bool SPSC<T, Alloc>::pop(T& value) {
